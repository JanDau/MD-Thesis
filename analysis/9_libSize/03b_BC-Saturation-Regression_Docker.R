...
g <- ggplot(data = df) +
  ...
  stat_function(fun = myfun, args = list(m = m_Chao), colour = "#DC0000", linewidth = rel(1)) +
...
g <- g + ylab("Unique sequences") +
  ...
  scale_y_continuous(limits = c(0, max(df)+0.05*max(df)), breaks = seq(0,280000,10000), labels = lab_indiv(0,280000,10000,50000, "dec")) +
...
#ggsave(file=ifelse(grepl("[hH]uman", usr_file), "Human-Regression.tiff", "Mouse-Regression.tiff"),
# ggsave(file="Mouse-Regression.tiff", height=6, width=6, units="cm", compression="lzw", dpi = 300)


outputFileName <- paste0("Mouse-Regression.tiff")
