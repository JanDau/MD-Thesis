...
# -----------------------
  ...
  usr[["thresh.label"]] <- 0.05 # Seq. higher than given perc. (as decimal) receive a label in the plot
  usr[["minimalistic"]] <- TRUE # TRUE = only stacked bars, FALSE = labels for Axis, Percentages, Seq & Read Count
  usr[["scale"]] <- 0.5 # y-scale for plot (as decimal)
  usr[["order"]] <- "overall" # sequence stack order based on "first", "last", "overall"
  usr[["bar.width"]] <- 0.5 # width of columns, values between 0.00 and 1.00
  usr[["width"]] <- 944 # width of plot in cm, set NA for automatic
  usr[["height"]] <- 413 # height of plot in cm, set NA for automatic
  usr[["areas"]] <- TRUE # if areas between stacks should be drawn
  usr[["area.last"]] <- TRUE # set TRUE if last bar should be connected with an area, too
  usr[["areas.alpha"]] <- 0.5 # transparency of connecting areas (between 0 and 1)
  usr$colors[["use.previous"]] <- TRUE # if you run the code for the same animal with different samples (TRUE or FALSE)
